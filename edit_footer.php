...
if (isset($_POST['update_footer'])) {
    ...
    // Pastikan footer_text tidak kosong
    if (!empty($footer_text)) {
        // Update footer header
        $stmt = $conn->prepare("UPDATE footer_header SET project = ? WHERE id = 1");
        $stmt->bind_param("s", $footer_text);
        if ($stmt->execute()) {
            $stmt->close();
            header("Location: edit_footer.php?status=footer_updated"); // Redirect setelah update
            exit();
        } else {
            echo "Error updating footer: " . $conn->error;
        }
    } else {
        echo "Footer text cannot be empty.";
    }
}
...
<div class="main-container">
    <nav>
        ...
        <button onclick="location.href='edit_header.php'" class="btn btn-primary btn-sm mt-2 w-100">Edit Header</button>
    ...
    <div class="mb-3">
        ...
        <input type="text" id="footer_text" name="footer_text" class="form-control" value="<?php echo htmlspecialchars($footer_data['project']); ?>" required>
    </div>
    ...
    <aside>
        <h3>Info</h3>
        <p>Halaman ini digunakan untuk memperbarui informasi footer yang tampil pada semua halaman.</p>
    </aside>
</div>
